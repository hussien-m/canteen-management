            ...
            <div class="card-content collpase show">
                <div class="card-body">
                        ...
                        <form class="form" action="{{route('roles.store')}}" method="post" enctype="multipart/form-data">
                            ...
                            @method('post')
                            <div class="form-body">
                                <h4 class="form-section"><i class="ft-user"></i>اضافة دور</h4>
                                <div class="row">
                                    <div class="form-group col-md-6 mb-2">
                                        <label for="projectinput1">اسم الدور</label>
                                        <input type="text" id="projectinput1" class="form-control" placeholder="اسم الدور" name="name" value="{{old('name')}}">
                                    </div>
                                </div>
                                                            ...
                                                            <div role="tabpanel" class="tab-pane {{$index == 0 ? 'active':''}}" id="{{$permission}}" aria-expanded="true" aria-labelledby="base-tab11">
                                                                ...
                                                                @foreach(config('permissions.'.$permission) as $code => $label)
                                                                    <fieldset class="checkboxsas">
                                                                        <label>
                                                                            <input type="checkbox" name="permissions[]" value="{{$code}}"> {{$label}}
                                                                        </label>
                                                                    </fieldset>
                                                                @endforeach
